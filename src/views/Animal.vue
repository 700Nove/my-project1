    ...
    methods: {
        ...
        handleAdd(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    this.isSubmit = true;
                    Request.post("/animal", this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '添加成功',
        ...
        handleEditSave(formName) {
            this.$refs[formName].validate(valid => {
                if (valid) {
                    this.isSubmit = true;
                    Request.put("/animal/" + this.dialogForm.id, this.dialogForm).then(response => {
                        if (response.code == 0) {
                            this.$message({
                                showClose: true,
                                message: '更新成功',
        ...
        exportTable(type) {
            if (this.tableData.length) {
                const params = {
                    header: ['宠物头像', '宠物名字', '宠物种类', '宠物性别', '宠物年龄', '宠物状态', '宠物品种', '宠物介绍'],
                    key: ['img', 'name', 'type', 'sex', 'age', 'status', 'breed', 'descr'],
